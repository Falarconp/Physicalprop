...
def intensity(nu, T, nu0, alpha, nco, vturb, angle, i0, hdr, iso=12):
    ...
    thermal = i0*scaling*sp.exp(-tau(T,nu,nu0,nco,vturb,angle, iso))*(nu/nu0)**alpha
...
# iminuit least squares fit
            ...
            f = lambda Temp,i0,vturb,NcolCO: flux_int(Temp*100., alpha, i0*1e-3, 10**19*NcolCO,
                                                       500.*vturb, angle, data, head12,
                                                       head13, head18)
            m = Minuit(f, Temp=0.2, i0=0.05, vturb=0.3, NcolCO=0.01, errordef=stderr,
                        error_Temp=.03, error_i0=0.01, error_NcolCO=0.005, error_vturb=0.02,
                        limit_Temp=(0.01,1), limit_i0=(0.001,1), limit_vturb=(0, 1),
                        limit_NcolCO=(0.001, 1))

            m.migrad(1e6)
            fit = [m.values['Temp']*100., m.values['vturb']*500., 10**19*m.values['NcolCO']];
            error = [m.errors['Temp']*100., m.errors['vturb']*500., 10**19*m.errors['NcolCO']];
            errmod = f(fit[0], m.values['i0'], fit[1], fit[2])
...
#    pool = Pool(3)
#    pool.map(iterate, zip(range(256),range(256)))
...
input13CO = '/home/felipe/Escritorio/ID/HD142527/Simulations_CO3-2/Vortex_image_12co32_o500_i159.00_phi90.00_PA-24.00.fits_MOM0'
input12CO = '/home/felipe/Escritorio/ID/HD142527/Simulations_CO3-2/Vortex_image_13co32_o500_i159.00_phi90.00_PA-24.00.fits_MOM0'
inputC180 = '/home/felipe/Escritorio/ID/HD142527/Simulations_CO3-2/Vortex_image_c18o32_o500_i159.00_phi90.00_PA-24.00.fits_MOM0'


minprofilesfitmoment(input12CO, input13CO, inputC180, r=128)
